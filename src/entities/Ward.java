...
public class Ward 
{
	...
	public String displayEmptyBed() {
		String emptyBed = "";
		for (int i = 0; i < beds.length; i++) {
			if (beds[i]  == null) {
				int label = i + minBedLabel;
				emptyBed += "	No. " + label + " is empty.\n";
			}
		}
		return "DISPLAY ALL EMPTY BEDS:\n" + emptyBed + "in the" + getName();
	}

	public String displayUsedBed() {
		String check = "";
		for (int i = 0; i < beds.length; i++) {
			if (beds[i]  != null) {
				int label = i + minBedLabel;
				check += "	No. " + label + " is Used. -- " + beds[i].getName() + "\n";
			}
		}
		return "SHOW ALL USED BEDS:\n" + check + "in the" + getName();
	}
